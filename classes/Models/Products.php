...
class Products extends Admin
{
	...
	public function GetProductsList(Request $data)
	{
		...
		$total_pages_sql = $this->query->select_join(
			"COUNT(*)",
			self::TABLE_PRODUCTS,
			[
		...
		$s = $this->query->select_join(
			"p.id,p.seo_name, p.sku as code,p.description,p.tags, p.name, stoc.quantity as existence, pric.normal as price",
			self::TABLE_PRODUCTS,
			[
				self::TABLE_STOCK => "stoc.product_id = p.id",
				self::TABLE_PRICES => "p.id = pric.product_id",
			...
			$f,
			"GROUP BY p.id ORDER BY p.id LIMIT " . $offset . "," . $this->no_of_records_per_page
		);
	...
	public function GetProductsList2($data)
	{
		...
		$total_rows = $result['COUNT(*)'];
		$total_pages = ceil($total_rows / $this->no_of_records_per_page);
		$offset = ($page - 1 < 0 ? 0 : $page - 1) * $this->no_of_records_per_page;
		$s = $this->query->select_join(
			"p.id,p.seo_name, p.sku as code,p.description,p.tags, p.name, stoc.quantity as existence, pric.normal as price",
			self::TABLE_PRODUCTS,
			[
	...
	public function GetProductDetails($data)
	{
		...
		$s = $this->query->select_join(
			"p.id, p.sku as sku,p.description, p.short_description, p.name, p.advantages, pric.normal as price, stoc.quantity as existence",
			self::TABLE_PRODUCTS,
			[
				self::TABLE_STOCK => "stoc.product_id = p.id",
	...
	{
		$s = $this->query->select_join(
			"p.id, p.sku as code,p.description,p.tags, p.name, stoc.quantity as existence, pric.normal as price",
			self::TABLE_PRODUCTS,
			[
				self::TABLE_STOCK => "stoc.product_id = p.id",
				self::TABLE_PRICES => "p.id = pric.product_id"
	...
	public function GetProductCustomData(Request $request)
	{
		$s = $this->query->select_join(
			...
			"p.product_id = " . $request->get("product_id"),
			"p.id"
		);
		return $this->GetAllRows($s);
	}
